			...
			<div class="shop-item row-between-center" v-for="(item,key) in shopList" :key="key">
				...
				<div class="shop-item-info">
					<div class="row-start-center">
						...
						<a v-if="item.pass" style="color: red;margin-left: 10px" @click="fans_param.page=1;fans_param.shopid=item._id;getFans();showFans=true;">查看粉丝</a> 
					</div>
					<div class="info-edit">
						是否上架：
					...
					<div class="info-edit">
						<a class="edit" @click="$router.push(`/user/5?_id=${item._id}`)">所有订单</a>
						|
						<a class="edit" @click="$router.push(`/user/5?_id=${item._id}&unfinished=1`)">未完成(<b>{{ orderCount[item._id] }}</b>)
						</a>
					</div>
				</div>
			</div>
